...
func run() error {
	...
	slog.InfoContext(ctx, "Starting show_meeting_attendees migration",
		"nats_url", *natsURL,
		"bucket", *bucketName,
...
func processRecord(ctx context.Context, kvStore jetstream.KeyValue, uid string, dryRun bool, indexSubject string, nc *nats.Conn) error {
	...
	if _, exists := dataMap["show_meeting_attendees"]; exists {
		slog.DebugContext(ctx, "record already has show_meeting_attendees field",
			"uid", uid,
		)
	...
	dataMap["show_meeting_attendees"] = false
	dataMap["updated_at"] = time.Now().Format(time.RFC3339Nano)

	slog.DebugContext(ctx, "setting show_meeting_attendees",
		"uid", uid,
		"value", false,
	)
