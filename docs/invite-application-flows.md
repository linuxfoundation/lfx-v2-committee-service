...
- **Blocked when expired:** a `pending`/`declined` invite whose `expires_at` (`created_at + 30 days`) has passed is rejected with `409 Conflict` ("invite has expired"), keeping in-app acceptance consistent with the email link, whose token shares the same TTL and is already dead. Legacy records with a zero `expires_at` are never treated as expired. The check runs after the `accepted`-status short-circuit, so an already-accepted invite still returns its member idempotently regardless of expiry.
