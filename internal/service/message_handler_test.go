...
// TestAcceptPendingCommitteeInvites verifies that HandleInviteAccepted accepts pending committee
// invites (updates status to "accepted") when the port-level committeeWriter is wired.
func TestAcceptPendingCommitteeInvites(t *testing.T) {
	ctx := context.Background()
	committeeUID := uuid.New().String()
	inviteUID := "test-invite-1"
	writerEmail := "writer@example.com"
	username := "testuser"

	newCommittee := func(uid string) *model.Committee {
		return &model.Committee{
			CommitteeBase:     model.CommitteeBase{UID: uid, ProjectUID: "proj-1", Name: "Test Committee"},
			CommitteeSettings: &model.CommitteeSettings{UID: uid},
		}
	}
	makeAcceptEvent := func() []byte {
		event := inviteapi.InviteServiceAcceptedEvent{Invite: inviteapi.Invite{
			UID:        uuid.New().String(),
			AcceptedBy: username,
	...
	t.Run("pending invite — status updated to accepted and indexer published", func(t *testing.T) {
		mockRepo := mock.NewMockRepository()
		mockRepo.ClearAll()
		mockRepo.AddCommittee(newCommittee(committeeUID))
		mockRepo.AddCommitteeInvite(&model.CommitteeInvite{
			UID:          inviteUID,
			CommitteeUID: committeeUID,
			InviteeEmail: writerEmail,
			Status:       "pending",
		...
		handler := NewMessageHandlerOrchestrator(
			...
			WithCommitteeWriterOrchestratorForMessageHandler(&spyCommitteeWriterOrchestrator{}),
			WithCommitteeWriterForMessageHandler(mock.NewMockCommitteeWriter(mockRepo)),
			WithCommitteePublisherForMessageHandler(pub),
		)

		msg := newMockTransportMessenger(inviteapi.InviteServiceAcceptedSubject, makeAcceptEvent())
		_, err := handler.HandleInviteAccepted(ctx, msg)
		require.NoError(t, err)

		// Verify invite status was updated to accepted in storage
		updated, _, getErr := mockRepo.GetInvite(ctx, inviteUID)
		require.NoError(t, getErr)
		assert.Equal(t, "accepted", updated.Status, "invite status must be updated to accepted")

		// Verify indexer was published for the invite update
		assert.GreaterOrEqual(t, pub.indexerCallCount, 1, "indexer must be published after invite acceptance")

		// Verify FGA invitee tuple was published
		require.Len(t, pub.capturedAccessSubjects, 1, "FGA invitee grant must be published")
		assert.Equal(t, fgaconstants.GenericUpdateAccessSubject, pub.capturedAccessSubjects[0])
	})

	t.Run("already-accepted invite — FGA still published, UpdateInvite not called again", func(t *testing.T) {
		mockRepo := mock.NewMockRepository()
		mockRepo.ClearAll()
		mockRepo.AddCommittee(newCommittee(committeeUID))
		mockRepo.AddCommitteeInvite(&model.CommitteeInvite{
		...
		// Invite should remain accepted and indexer should NOT be called again
		updated, _, getErr := mockRepo.GetInvite(ctx, inviteUID)
		require.NoError(t, getErr)
		assert.Equal(t, "accepted", updated.Status, "already-accepted invite status must not change")
		assert.Equal(t, 0, pub.indexerCallCount, "indexer must not be re-published for already-accepted invite")

		// FGA must still be published so the user retains access
		require.Len(t, pub.capturedAccessSubjects, 1, "FGA invitee grant must still be published")
	})
}
