...
func PublicNameAttribute() {
	dsl.Attribute("public_name", dsl.String, "Human-readable slug for public URLs (e.g. 'tsc'); defaults to the committee name when public is enabled", func() {
		dsl.MaxLength(200)
		dsl.Example("technical-steering-committee")
	})
}
...
func XSyncAttribute() {
	dsl.Attribute("x_sync", dsl.Boolean, "Requests synchronous processing for applicable downstream operations, including indexer messages. FGA update_access and delete_access publications remain asynchronous and do not wait for FGA processing or OpenFGA convergence.", func() {
		dsl.Default(false)
		dsl.Example(true)
	})
}
